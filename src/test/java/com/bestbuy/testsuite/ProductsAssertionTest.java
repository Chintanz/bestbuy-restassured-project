...
@Test
public void test004(){
    response.body("data.findAll{it.name == 'name'}", hasItem(hasEntry("Duracell", "Energizer")));
}
...
//17. Verify the price = 4.99 of forth product
    @Test
    public void test017(){
        List<Double> price = response.extract().path("data.findAll{it.name == 'Energizer - MAX Batteries AA (4-Pack)'}.price");
    }
...
@Test
public void test020(){
    List<HashMap<String, ?>> categories  = response.extract().path("data[9].categories");
